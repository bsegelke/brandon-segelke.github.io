...
var palindrome = function(string, outputrev = [], output = string) {
  ...
  if(string.length === 0 && outputrev.join('').toLowerCase().replace(' ', '') === output.toLowerCase().replace(' ', '')){
    
    return true
  }else if(string.length === 0 && outputrev.join('').toLowerCase().replace(' ', '') !== output.toLowerCase().replace(' ', '')){
    return false
  }
...
var compareStr = function(str1, str2, result = true) {
  ...
  if(str1[0] === str2[0]){
      console.log(str1[0])
      result = true;
      return compareStr(str1.slice(1), str2.slice(1), result)
  }else{
      return false;
  }
...
function nthFibo(n) {
  ...
  if(n === 0) return 0;
  //if n is 2 or less than return 1
  if (n <= 2) return 1;
// return n-1 + n-2
